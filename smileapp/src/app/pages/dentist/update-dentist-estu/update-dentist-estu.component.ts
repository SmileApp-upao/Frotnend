...
@Component({
  selector: 'app-update-dentist-estu',
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule],
  templateUrl: './update-dentist-estu.component.html',
  styleUrl: './update-dentist-estu.component.scss'
})
export class UpdateDentistEstuComponent {
  ...
  constructor() {
    this.updateDentistForm = this.fb.group({
      ...
      cicle: ['', [Validators.required, Validators.min(5), Validators.max(10)]],
      description: [''],
    });
  }
  ...
  loadUserProfile(): void {
    ...
    if(userId) {
      ...
      this.dentistService.getEstudentProfile(userId).subscribe({
          next: (dentistProfile) => {
  ...
  onSubmit(): void {
    ...
    if(this.updateDentistForm.valid) {
      ...
      if(userId) {
        ...
        this.dentistService.updateEstudentDentist(userId, updatedDentist).subscribe({
            next: () => {
